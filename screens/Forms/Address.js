...
const Address = ({ navigation }) => {
	...
	const onAlertTriggered = (message) => {
		return Alert.alert("User Profile.!", message, [
	...
	function onSubmit() {
		if (formState.first_name !== undefined && formState.first_name.length > 2) {
			if (formState.pincode !== undefined && formState.pincode.length == 6) {
				...
				} else onAlertTriggered("Please enter full address with house details.");
			} else onAlertTriggered("Please enter six digit Pin code");
		} else onAlertTriggered("Please enter proper full name");
	}
		...
		<View style={styles.container}>
			<View style={styles.sub_container}>
				<ScrollView>
					<View style={{ flex: 1, flexDirection: "row", padding: 6 }}>
						<Text style={{ ...FONTS.big_button_text, fontWeight: "bold", marginLeft: 6 }}>
							User Profile:
						</Text>
					</View>
					<View
						style={{ flex: 3, flexDirection: "column", marginLeft: 5, marginRight: 5 }}
					>
					...
					<View style={{ flex: 1, flexDirection: "row" }}>
						<Text style={{ marginLeft: 20, fontStyle: "italic", marginTop: 0 }}>
							**PIN yang akan digunakan setiap melakukan transaksi
						</Text>
					</View>
				</ScrollView>
			</View>
			<View style={styles.submit_btn_container}>
				<TouchableOpacity onPress={onSubmit}>
					<Text style={styles.submit_box}>S I M P A N</Text>
				</TouchableOpacity>
			</View>
		</View>
	);
